        ...
        for episode in range(params.num_episodes):
            ...
            for n_task in range(num_inner_tasks):
                ...
                with torch.no_grad():  # Temporarily disable gradient tracking
                    for param, grad in zip(net_clone.parameters(), grads0):
                        param += ((0.1/task_lr)/float(num_inner_tasks)) * grad
                with torch.no_grad():  # Temporarily disable gradient tracking
                    for param, grad in zip(net_clone.parameters(), grads1):
                        param -= ((1+(0.1/task_lr))/float(num_inner_tasks)) * grad
                # meta_loss += loss_t1
                net_clone0.load_state_dict(a_dict)
            ...
            # meta_loss.backward()
            # meta_optimizer.step()
            adapted_dict = net_clone.cloned_state_dict()
