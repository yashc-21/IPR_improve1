...
def evaluate(model, loss_fn, meta_classes, task_lr, task_type, metrics, params, args,
    ...
    for episode in range(num_steps):
        ...
        dataloaders = fetch_dataloaders(['train', 'test'], task, params)
        dl_sup = dataloaders['train']
        dl_que = dataloaders['test']
        X_sup, Y_sup = next(iter(dl_sup))
        X_que, Y_que = next(iter(dl_que))
...
if __name__ == '__main__':
    ...
    restore_path = './model_checkpoint.pth.tar'
    ...
    checkpoint_data = torch.load(restore_path)
    ...
    save_dir = "experiments/emodb_ravdess_tess/shemo"
